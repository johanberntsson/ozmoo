...
{
    // BAM and label
    write (fdo1, track1801, 256);
}

void
trans1802 ()
{
    // empty directory track with no files listed
...
main (int argc, char **argv)
{
  ...
  if ((version == 4) || (version == 5))
    ...
    transzero (21); /* track 17 */
    trans1801(); /* track 18 */
    trans1802();
    // the rest if empty (free to use)
    transzero (17);
    for (i = 19; i < 36; i++) {
        transdata (17, 1);
        if (i <= 30) transzero (1);
  ...
  if (read (fdi2, databuf, 256) != 0) 
    ex ("datafile longer than 16*16 sectors, create failed");
#endif
