...
- `-fcm` (`Z6_FCM_MODE`, MEGA65 + v6 only) puts the VIC-IV into Full Colour Mode with 16-bit character codes: 320x200, 40x25 cells. Codes below 256 stay ordinary glyphs (`FCLRLO` clear), codes from 256 up are 64-byte tiles. It should render text identically to the C64. The z6 kernal has no 80-column text path — `s_printchar` always writes two-byte FCM cells — so everything, including the **splash screen**, renders in FCM; the splash lines are centred for 40 columns, so `splashscreen.asm` skips the +20 the 80-column MEGA65 build adds to re-centre them (which on the 40-column screen would wrap into garbage). Per-window background colours are **not** implemented yet, which is the remaining half of the FCM work. On quit, `z_ins_quit` clears the FCM mode bits (`$d054` CHR16/FCLRHI) and the mouse sprite before the C64 reset, which never touches `$d054` — otherwise BASIC comes up in 16-bit-character full-colour mode, unreadable.
