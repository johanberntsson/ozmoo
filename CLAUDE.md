...
- **VICE's `-80col` does not reach the kernal's 40/80 flag** when `-default` is also passed (and `-default` after it resets it regardless). Ozmoo reads that flag, `$d7` (`COLS_40_80`), so a headless 80-column run is most reliably had by poking it at Ozmoo's entry: a tracepoint on `program_start` with `command N "> d7 80"`. Check you got it by dumping `$d7` at the read prompt — if it reads 0, the whole run was 40 columns and proved nothing about the VDC.
