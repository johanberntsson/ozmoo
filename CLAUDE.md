...
- **Picture numbers run to 999** (`pic_number_lo`/`_hi`, `rect_number_lo`/`_hi`; the `P###` filename is three digits). The per-build picture *count* (the load/search index) is still 8-bit, so a build holds at most 255 distinct pictures — `pics2asm.py` errors above that. Lifting it (Zork Zero has 396) means a 16-bit sequential index; see `todo.txt`.
...
- The pointer is a red arrow (sprite 0 colour, `mouse.asm`), chosen so it stays visible over the white status line. Verifying the actual click needs a real mouse: headless xemu cannot move or press one, though the sprite, `mouse_active`, and the terminator table can all be read back.
...
- **A big picture set spans several disks now, but the count is still capped at 255.** The `-pics` pipeline packs the numbered files across as many `_pics_N.d81` disks as they need (see the Pictures section) and `pic_load_all` sweeps the disks at boot, trying the second drive before asking for a swap. Picture *numbers* go up to 999, but the per-build picture *count* — the load/search index — is still 8-bit, so a build holds at most 255 pictures and `make zork0` still runs `-fcm` **without** pictures (Zork Zero has 396). Lifting that is a 16-bit sequential index over the five parallel `pic_*` tables; it's the next step and is deliberately deferred (it can't be runtime-tested here until a >255-picture build uses `-pics`). See `todo.txt`.
